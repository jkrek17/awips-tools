# ----------------------------------------------------------------------------
# OutOfDomainSpot_current.py
# Multi-model ensemble spot forecast tool with error bars
# Version 1.0 (STABLE VERSION - PRODUCTION READY)
# 
# This is the stable, tested version. Use this for production.
# Working version: OutOfDomainSpot.py
# ----------------------------------------------------------------------------
...
class Procedure(SmartScript.SmartScript):
    ...
    def _retrieve_multi_model_ensemble(self, config, time_range):
        ...
        for model_name, atmo_model, wave_model, atmo_level, wave_level in models:
            ...
            try:
                ...
                LogStream.logEvent(f"{model_name}: {len(cycles)} cycles")
                ...
                for cycle_str in cycles:
                    ...
                    for grid in atmo_cycles[cycle_str]:
                        try:
                            ...
                            if u_val is None or v_val is None:
                                continue
                            u = float(u_val) * 1.94384
                            v = float(v_val) * 1.94384
                    ...
                    for grid in wave_cycles[cycle_str]:
                        try:
                            ...
                            # Ensure values are numeric
                            if htsgw_val is None or perpw_val is None or dirpw_val is None:
                                continue
    ...
    def _calculate_ensemble_stats(self, ensemble_data):
        ...
        for idx, valid_dt in enumerate(sorted_times):
            ...
            if data['wind_speeds']:
                ...
                speed_mean = float(np.nanmean(speeds)) if len(speeds) > 0 else np.nan
                dir_mean = float(np.nanmean(dirs)) if len(dirs) > 0 else np.nan
                gust_mean = float(np.nanmean(gusts)) if len(gusts) > 0 else np.nan
                
                if np.isfinite(speed_mean) and np.isfinite(dir_mean):
                    u, v = self.MagDirToUV(speed_mean, dir_mean)
                else:
                    u, v = np.nan, np.nan
                
                model_data_dict['speed'][fake_cycle][idx] = speed_mean
                model_data_dict['speed_min'][fake_cycle][idx] = float(np.nanmin(speeds)) if len(speeds) > 0 else np.nan
                model_data_dict['speed_max'][fake_cycle][idx] = float(np.nanmax(speeds)) if len(speeds) > 0 else np.nan
                model_data_dict['gust'][fake_cycle][idx] = gust_mean
                model_data_dict['gust_min'][fake_cycle][idx] = float(np.nanmin(gusts)) if len(gusts) > 0 else np.nan
                model_data_dict['gust_max'][fake_cycle][idx] = float(np.nanmax(gusts)) if len(gusts) > 0 else np.nan
            ...
            if data['wave_heights']:
                ...
                if len(waves) > 0:
                    model_data_dict['htsgw'][fake_cycle][idx] = float(np.nanmean(waves))
                    model_data_dict['htsgw_min'][fake_cycle][idx] = float(np.nanmin(waves))
                    model_data_dict['htsgw_max'][fake_cycle][idx] = float(np.nanmax(waves))
                else:
                ...
                if len(dirs) > 0:
                    dirpw_mean = float(np.nanmean(dirs))
                ...
                if len(periods) > 0:
                    model_data_dict['perpw'][fake_cycle][idx] = float(np.nanmean(periods))
                else:
    ...
    def drawMeteogram(self, times, speeds, speeds_min, speeds_max, gusts, gusts_min, gusts_max,
        ...
        best_loc = find_best_legend_position(times_w, speeds, speeds_min, speeds_max)
        if has_gusts:
        ...
        if model_sys == 'Multi-Model':
            title = 'Significant Wave Height - Multi-Model Ensemble Mean'
        else:
            title = 'Significant Wave Height - GFE with Ensemble Spread'
        
        # Find best legend position based on data density
        best_loc = find_best_legend_position(times_wv, waves_plot, waves_min_plot, waves_max_plot)
        
        ax3.legend(loc=best_loc, framealpha=0.9, 
                  fontsize=9, facecolor='white', edgecolor='gray')
        ax3.set_title(title, fontweight='bold', fontsize=12, pad=5, loc='left')
