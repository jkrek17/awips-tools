    ...
    def execute(self, editArea, timeRange, varDict=None):
        ...
        for gridinfo in gridinfos:
            ...
            if totweight > 0:
                ...
                # Apply smoothing if requested - use simple grid smoothing
                if applySmoothing == "Yes":
                    for _ in range(smoothingPasses):
                        blendedMag = self.smoothGrid(blendedMag)
                        blendedDir = self.smoothGrid(blendedDir) 
                        windGust = self.smoothGrid(windGust)
                    blendedWind = (blendedMag, blendedDir)
                ...
                # Save the blend and gust for this time period
            ...
            if createWaves == "Yes" and len(waveSources) > 0:
                ...
                if wavetotweight > 0:
                    ...
                    if applySmoothing == "Yes":
                        for _ in range(smoothingPasses):
                            waveMag = self.smoothGrid(waveMag)
