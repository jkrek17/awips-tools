...
class CONFIG:
    ...
    QPF_HEAVY = .5            # Heavy intensity threshold (inches)
    QPF_MODERATE = 0.25          # Moderate intensity threshold (inches)
    QPF_LIGHT = 0.05             # Light intensity threshold (inches)
    
    # QPF thresholds for coverage/probability terms
    QPF_WIDE_COVERAGE = .25    # Wide coverage threshold (inches)
    QPF_NUMEROUS = 0.10          # Numerous coverage threshold (inches)
    QPF_SCATTERED = 0.03        # Scattered coverage threshold (inches)
    # Below SCATTERED = Isolated
    
    # QPF thresholds for probability terms (stratiform)
    QPF_DEFINITE = .25        # Definite probability threshold (inches)
    QPF_LIKELY = 0.10           # Likely probability threshold (inches)
    QPF_CHANCE = 0.03            # Chance probability threshold (inches)
    ...
    CONVECTIVE_THRESHOLD = 7  # Above = convective/coverage, Below = stratiform/probability
    ...
    SMOOTHING_DEFAULT = 10       # Default smoothing factor (0-5)
    SMOOTHING_MIN = 0           # Minimum smoothing (0 = none)
    SMOOTHING_MAX = 20           # Maximum smoothing (5 = heavy)
    ...
    def execute(self, editArea, timeRange, varDict=None):
        ...
        for i, gridinfo in enumerate(gridinfos):
            ...
            for model in atmModels:
                ...
                # Get QPF (precipitation)
                try:
                    qpf_grid = self.getGrids(atmDataBaseID, "tp", "SFC", GridTimeRange, mode="First", noDataError=0)
                    if qpf_grid is not None:
                        ...
                        # tp is typically cumulative from model start, but mode="First" should give us
                        # the grid valid for this time period
            ...
            if createDiagnostics == "Yes":
                ...
                try:
                    if qpf_avg is not None:
                        ...
                        self.createGrid("Fcst", "modelQPF", "SCALAR", qpf_clipped, GridTimeRange, minAllowedValue=0.0, maxAllowedValue=1.0)
                ...
                try:
                    if cape_avg is not None:
                        ...
                        self.createGrid("Fcst", "modelCAPE", "SCALAR", cape_clipped, GridTimeRange, minAllowedValue=0.0, maxAllowedValue=5000.0)
                ...
                try:
                    if temp_avg is not None:
                        ...
                        self.createGrid("Fcst", "modelT", "SCALAR", temp_f_clipped, GridTimeRange, minAllowedValue=0.0, maxAllowedValue=100.0)
                ...
                try:
                    if rh_avg is not None:
                        ...
                        self.createGrid("Fcst", "modelRH", "SCALAR", rh_clipped, GridTimeRange, minAllowedValue=0.0, maxAllowedValue=100.0)
                ...
                try:
                    if vis_avg is not None:
                        ...
                        self.createGrid("Fcst", "modelVsby", "SCALAR", vis_clipped, GridTimeRange, minAllowedValue=0.0, maxAllowedValue=10.0)
                ...
                try:
                    if cape_avg is not None and wind_avg is not None:
                        ...
                        self.createGrid("Fcst", "modelConvectiveIndex", "SCALAR", conv_index_clipped, GridTimeRange, minAllowedValue=0.0, maxAllowedValue=10.0)
            ...
            hasThunder = cape_avg > CONFIG.CAPE_THUNDER_MIN if cape_avg is not None else None
